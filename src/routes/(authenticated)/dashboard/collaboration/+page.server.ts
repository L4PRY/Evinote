...
export const load = async () => {
	...
	// Fetch boards where the user has specific permissions (shared with them)
	const boards = await db
		...
		.from(table.Board)
		.innerJoin(table.Permissions, eq(table.Permissions.bid, table.Board.id))
		.where(eq(table.Permissions.uid, user.id))
